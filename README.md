...
[Approov Demo Server](./docs/approov-shapes-demo-server.md) walk-through.
...
checks, and for this we need to add `ApproovSecurityContextRepository(approovConfig, checkCustomPayload)`
...
tell if the custom payload claim is to be checked or not, and this is done with
...
with `@Order(2)` we perform the optional check for the Approov custom payload
claim, and then with `@Order(3)` we proceed as usual, that in this demo is to
...
Now let's protect the endpoint for `/shapes` and `/forms` with an Approov token.

The `/shapes` endpoint it will be protected only by the required checks for an
Approov token, while the `/forms` endpoint will have the optional check for the
...
`/shapes` to be reached without any authentication.


#### Protecting the `/forms` endpoint

This endpoint also requires that we perform the optional check for the custom
payload claim, thus to protect the `/forms` endpoint another subclass is
necessary.
...
`/forms` to be reached without any authentication.


#### Putting All-Together

After we implemented the Approov protection for the `/shapes` and `/forms`
